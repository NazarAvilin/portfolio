<?php
	...
	if ($_POST) {
         ...
         if (!$_POST['email']) {
            ...
            $error .= 'Поле электронной почты обязательно!<br>';

        }

        if (!$_POST['subject']) {

            $error .= 'Поле темы обязательно!<br>';

        }

        if (!$_POST['content']) {

            $error .= 'Поле контента обязательно!<br>';

        }

        if ($_POST['email'] && filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) === false) {
            
            $error .= 'Адрес э.почты не действителен!<br>';

        }

        if ($error != "") {

            $error = '<div class="alert alert-danger" role="alert"><p><strong>Присутствуют ошибки в вашей форме:</strong></p>'.$error.'</div>';
              ...
              if (mail($emailTo, $subject, $content, $headers)) {
               ...
               $successMsg = '<div class="alert alert-success" role="alert"><strong>Ваше сообщение отправлено, скоро вам отвечу!</strong></div>';

              } else {

                $error = '<div class="alert alert-danger" role="alert"><p><strong>Ваше сообщение не отправлено, попробуйте в другое время!</strong></p></div>';
...
<html lang="en">
  <head>
    ...
    <meta charset="utf-8">
    <meta name="google-site-verification" content="9XHuvMdyjaJKq1XKX2I-30QAwswvkphMGjyXFbgvAlg" />
    <meta name="yandex-verification" content="9334bc767756f2ed" />
  ...
  <section class="jumbotron center" id="contactMe">
    ...
    <div class="container center">
      ...
      <h2 class="title write-me">Пишите Мне!</h2>

      <div id="error"><?php echo $error.$successMsg; ?></div>

      <form method="POST">
          <div class="form-group row">
            <label for="email" class="col-sm-2 col-form-label text-left title">Адрес Эл.почты</label>
            <div class="col-sm-8">
          ...
          <div class="form-group row">
            <label for="subject" class="col-sm-2 col-form-label text-left title">Тема</label>
            <div class="col-sm-8">
          ...
          <div class="form-group row">
            <label for="content" class="col-sm-2 col-form-label text-left title">Ваше Сообщение</label>
            <div class="col-sm-8">
              <textarea class="form-control" id="content" rows="7" name="content"></textarea>
            </div>
          </div>

          <button type="submit" class="btn btn-primary col-sm-2 title">Отправить</button>
    ...
    <script type="text/javascript">
        ...
        $('form').submit( function(){
            ...
            if (error != '') {
                ...
                $('#error').html('<div class="alert alert-danger" role="alert"><p><strong>Присутствуют ошибки в вашей форме: </strong></p>'+error+'</div>');
                return false;
